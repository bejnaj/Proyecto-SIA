...
public class MenuPrincipalUI extends JFrame {
    ...
    public MenuPrincipalUI(Estudiante usuario) {
        ...
        JButton btnListar = new JButton("Lista de Recursos por materia");
        JButton btnPrestar = new JButton("Solicitar recurso");
        ...
        JButton btnListarPoseedores = new JButton("Lista de poseedores");
        ...
        panelBotones.add(btnEliminarEstudiante);
        panelBotones.add(btnBuscarPorEstudiante); 
        panelBotones.add(btnGenerarReporte); 
        panelBotones.add(btnSalir);

        /* ROLE FILTERS */
        boolean isAdmin = usuario.getRut().equalsIgnoreCase("admin") && usuario.getNombre().equalsIgnoreCase("admin");
        if (!isAdmin) {
            // Deshabilitar opciones restringidas para alumnos con tooltip
            btnAgregar.setEnabled(false);
            btnAgregar.setToolTipText("Sólo disponible para el administrador");

            btnEditarRecurso.setEnabled(false);
            btnEditarRecurso.setToolTipText("Sólo disponible para el administrador");

            btnEliminarRecurso.setEnabled(false);
            btnEliminarRecurso.setToolTipText("Sólo disponible para el administrador");

            btnListarPoseedores.setEnabled(false);
            btnListarPoseedores.setToolTipText("Sólo disponible para el administrador");

            btnEditarEstudiante.setEnabled(false);
            btnEditarEstudiante.setToolTipText("Sólo disponible para el administrador");

            btnEliminarEstudiante.setEnabled(false);
            btnEliminarEstudiante.setToolTipText("Sólo disponible para el administrador");

            btnBuscarPorEstudiante.setEnabled(false);
            btnBuscarPorEstudiante.setToolTipText("Sólo disponible para el administrador");

            btnGenerarReporte.setEnabled(false);
            btnGenerarReporte.setToolTipText("Sólo disponible para el administrador");
        }
        ...
        btnAgregar.addActionListener(e -> {
            try {
                ...
                JOptionPane.showMessageDialog(this, "Entrada inválida (número).");
            } catch (Exception ex) { 
                ex.printStackTrace(); 
                JOptionPane.showMessageDialog(this, "Error agregando recurso."); 
            }
        });

        btnListar.addActionListener(e -> {
            String materiaBuscada = JOptionPane.showInputDialog(this, "Ingrese la materia a listar:"); 
            if (materiaBuscada==null) return;
            ...
            if (encontrados.isEmpty()) areaTexto.append("No se encontraron recursos para " + materiaBuscada + "\n");
                ...
                for (Recurso rec : encontrados) 
                    areaTexto.append("ID: " + rec.getId() + " | " + rec.getTitulo() + " (" + rec.getTipoUso() + "), Cupos: " + rec.getCupos() + "\n");
            }
        });

        btnPrestar.addActionListener(e -> {
            try {
                String idStr = JOptionPane.showInputDialog(this, "Ingrese ID del recurso a prestar:"); 
                if (idStr==null) return;
            ...
            catch (CuposInsuficientesException | RecursoNoEncontradoException ex) { 
                JOptionPane.showMessageDialog(this, ex.getMessage()); 
            }
            catch (Exception ex) { 
                ex.printStackTrace(); 
                JOptionPane.showMessageDialog(this, "Error en el préstamo."); 
            }
        });

        btnDevolver.addActionListener(e -> {
            try {
                String idStr = JOptionPane.showInputDialog(this, "Ingrese ID del recurso a devolver:"); 
                if (idStr==null) return;
                ...
                ex.printStackTrace(); 
                JOptionPane.showMessageDialog(this, "Error en la devolución."); 
            }
        });

        btnEditarRecurso.addActionListener(e -> {
            try {
                String s = JOptionPane.showInputDialog(this, "Ingrese ID del recurso a editar:"); 
                if (s==null) return;
                int id = Integer.parseInt(s.trim()); 
                Recurso r = GestorDatos.getRecursosMap().get(id);
                if (r == null) { 
                    JOptionPane.showMessageDialog(this, "Recurso no encontrado."); 
                    return; 
                }
                ...
                JOptionPane.showMessageDialog(this, "Cupos inválido."); 
            }
            catch (Exception ex) { 
                ex.printStackTrace(); 
                JOptionPane.showMessageDialog(this, "Error editando recurso."); 
            }
        });

        btnEliminarRecurso.addActionListener(e -> {
            try {
                String s = JOptionPane.showInputDialog(this, "Ingrese ID del recurso a eliminar:"); 
                if (s==null) return;
                int id = Integer.parseInt(s.trim()); 
                Recurso r = GestorDatos.getRecursosMap().get(id);
                if (r == null) { 
                ...
                ex.printStackTrace(); 
                JOptionPane.showMessageDialog(this, "Error eliminando recurso."); 
            }
        });

        btnListarPoseedores.addActionListener(e -> {
            try {
                String s = JOptionPane.showInputDialog(this, "Ingrese ID del recurso:"); 
                if (s==null) return;
                int id = Integer.parseInt(s.trim()); 
                Recurso r = GestorDatos.getRecursosMap().get(id);
                if (r == null) { 
        ...
        btnEliminarEstudiante.addActionListener(e -> {
            String rut = JOptionPane.showInputDialog(this, "Ingrese RUT del estudiante a eliminar:"); 
            if (rut==null) return;
            Estudiante est = GestorDatos.buscarEstudiantePorRut(rut.trim()); 
            if (est==null) { 
                JOptionPane.showMessageDialog(this, "Estudiante no encontrado."); 
        ...
        btnBuscarPorEstudiante.addActionListener(e -> {
            String rut = JOptionPane.showInputDialog(this, "Ingrese RUT del estudiante a buscar:"); 
            if (rut==null) return;
            Estudiante est = GestorDatos.buscarEstudiantePorRut(rut.trim()); 
            if (est==null) { 
                JOptionPane.showMessageDialog(this, "Estudiante no encontrado."); 
